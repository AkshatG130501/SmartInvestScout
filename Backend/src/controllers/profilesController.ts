...
export class ProfilesController {
  private static instance: ProfilesController;
  ...
  public static getInstance(): ProfilesController {
    if (!ProfilesController.instance) {
      ProfilesController.instance = new ProfilesController();
    }
  ...
  public async createUserProfile(req: Request, res: Response) {
    try {
      ...
      if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
        ...
        return res
          .status(500)
          .json({ message: 'Server configuration error: Missing Supabase environment variables' });
      }

      const newProfile = await this.profileService.createUserProfile(userId, profileData);
  ...
  public async deleteUserProfile(req: Request, res: Response) {
    try {
      ...
      const success = await this.profileService.deleteUserProfile(userId);
